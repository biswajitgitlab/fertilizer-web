...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-3xl border border-gray-100/80 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:border-emerald-300/80 dark:hover:border-emerald-500/40 transition-all duration-300 flex flex-col overflow-hidden h-full"
    >
      ...
      <Link to={`/products/${product.slug}`} className="relative block aspect-4/3 sm:aspect-square overflow-hidden bg-emerald-950/20">
        ...
        {discount > 0 && (
          <motion.div
            ...
            className="absolute top-3 right-3 bg-rose-600/90 backdrop-blur-md text-white font-black text-xs px-2.5 py-1 rounded-xl shadow-md border border-rose-400/30"
          >
            -{discount}% OFF
          </motion.div>
        )}
        ...
        <button
          ...
          className="absolute bottom-3 right-3 p-2.5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full shadow-lg transition-all duration-300 opacity-0 group-hover:opacity-100 cursor-pointer transform translate-y-2 group-hover:translate-y-0"
        >
