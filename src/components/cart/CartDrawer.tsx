        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <motion.div
              ...
              className="w-screen max-w-md bg-white dark:bg-slate-900 shadow-2xl flex flex-col justify-between border-l border-gray-100 dark:border-slate-800 text-gray-900 dark:text-white"
            >
              
              {/* Header */}
              <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-emerald-950 text-white">
          ...
          {items.length > 0 && (
            <div className="bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 text-white p-4 space-y-2 border-b border-emerald-800">
              ...
              <div className="w-full bg-white/20 h-2.5 rounded-full overflow-hidden p-0.5 backdrop-blur-xs">
                <motion.div
                  initial={{ width: 0 }}
          ...
          {items.length > 0 && (
            <div className="p-4 sm:p-6 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 space-y-4">
              
              {/* Coupon Form & Available Offers */}
              {couponCode ? (
                <div className="flex items-center justify-between p-2.5 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-bold text-emerald-800 dark:text-emerald-300">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-2.5">
                  <form onSubmit={handleApplyCoupon} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 text-xs bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 uppercase focus:outline-none focus:border-emerald-500 font-mono font-bold text-gray-900 dark:text-white"
                    />
                  ...
                  {/* Quick-Apply Tokens */}
                  <div className="bg-white dark:bg-slate-800/80 border border-gray-200/80 dark:border-slate-700 rounded-xl p-2.5 space-y-1.5 shadow-2xs">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-black text-gray-800 dark:text-slate-200 flex items-center gap-1">
                        <AnimatedSparkles size={14} className="text-amber-500" /> Available Discount Tokens
                      </span>
                    </div>
                    <div className="space-y-1.5 pt-0.5">
                      {[
                        ...
                        <div key={t.code} className="flex items-center justify-between text-[11px] bg-emerald-50/60 dark:bg-emerald-950/40 p-1.5 rounded-lg border border-emerald-100 dark:border-emerald-900/60">
                          <div>
                            <span className="font-mono font-bold text-emerald-900 dark:text-emerald-300 bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-800 mr-1.5">
                              {t.code}
                            </span>
                            <span className="text-gray-600 dark:text-slate-400 font-medium text-[10px]">{t.text}</span>
                          </div>
                          <button
                            type="button"
              ...
              <div className="flex items-center justify-center gap-2 text-[11px] text-gray-500 dark:text-slate-400">
                <AnimatedShield size={16} className="text-emerald-600 dark:text-emerald-400" />
