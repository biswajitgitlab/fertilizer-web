              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs border-t border-b border-slate-100 dark:border-slate-800/60 py-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-slate-600 dark:text-slate-400 font-medium truncate block">
                ...
                <div className="flex items-center justify-between pt-1">
                  <div>
                    <span className="text-[10px] text-slate-400 block font-bold uppercase">Cash Collected</span>
                  ...
                  <div>
                    {s.status === 'SETTLED_TO_BANK' ? (
                      <button
                        disabled
                        className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-bold inline-flex items-center gap-1.5 opacity-80 cursor-not-allowed"
                      >
                      ...
                      <button
                        ...
                        className={`px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition shadow-xs flex items-center justify-center gap-1.5 ${settlingId === s.id ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer active:scale-95'}`}
                      >
                        {settlingId === s.id ? (
                      ...
                      <td className="py-3.5 px-4 text-right">
                        {s.status === 'SETTLED_TO_BANK' ? (
                          ...
                          <button
                            onClick={() => handleSettle(s.id)}
                            disabled={settlingId === s.id}
                            className={`px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[11px] font-bold transition shadow-xs flex items-center justify-center gap-1.5 ml-auto ${settlingId === s.id ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer active:scale-95'}`}
                          >
                            {settlingId === s.id ? (
                              <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Reconciling...</>
                            ) : (
