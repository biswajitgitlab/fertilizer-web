...
import { ShieldCheck, Plus, Check, Save, UserCheck, Key, Lock, Sparkles } from 'lucide-react';
  ...
  return (
    <AdminLayout title="Enterprise Roles & Permissions">
      <div className="space-y-6">
        ...
        {/* Header Bar */}
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-3xl border border-slate-800/80 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 px-4 py-2 rounded-xl text-xs font-black transition-all shadow-md shadow-emerald-950/50 flex items-center gap-1.5 cursor-pointer"
            >
            ...
            return (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer relative overflow-hidden ${
                  isSelected
                    ? 'bg-gradient-to-b from-slate-900 via-slate-900 to-emerald-950/40 border-emerald-500/60 shadow-xl shadow-emerald-950/40 ring-1 ring-emerald-500/50'
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-slate-950 border border-slate-800 text-emerald-400">
                    {role.user_count} Members
                  </span>
                </div>
                <h3 className="text-base font-black text-white mb-1">{role.name}</h3>
                <p className="text-xs text-slate-400">
                  {role.permissions.length} active fine-grained permissions
                </p>
                {isSelected && (
                  <div className="mt-3 pt-2 border-t border-emerald-500/20 flex items-center gap-1 text-[11px] font-bold text-emerald-400">
        ...
        {activeTab === 'matrix' && selectedRole && (
          <div className="bg-slate-900/60 backdrop-blur-md rounded-3xl border border-slate-800/80 shadow-xl p-6 space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
              <div>
                <h3 className="text-lg font-black text-white flex items-center gap-2">
                  <span>Permission Matrix for:</span>
                  <span className="text-emerald-400">{selectedRole.name}</span>
                </h3>
              ...
              <button
                ...
                disabled={isSaving}
                className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-950/60 flex items-center gap-2 cursor-pointer disabled:opacity-50"
              >
            ...
            <div className="space-y-6">
              {Object.entries(groupedPermissions).map(([category, perms]) => (
                <div key={category} className="bg-slate-950/80 rounded-2xl p-4 border border-slate-800/60 space-y-3">
                  <div className="flex items-center gap-2 border-b border-slate-800/60 pb-2">
                    <Sparkles className="w-4 h-4 text-emerald-400" />
                    <h4 className="text-sm font-black text-slate-200 uppercase tracking-wide">{category} Module Access</h4>
                  </div>
                ...
                <tbody className="divide-y divide-slate-800/50 text-xs text-slate-200 font-medium">
                  {team.map((member) => (
                    <tr key={member.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-slate-300 font-semibold">{member.email}</td>
                      <td className="py-3.5 px-4">
                        <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider">
                          {member.role}
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
                        <select
                          ...
                          className="bg-slate-950 border border-slate-800 text-xs text-slate-200 rounded-xl px-2.5 py-1 focus:outline-none focus:border-emerald-500 cursor-pointer"
                        >
                          {roles.map((r) => (
        ...
        {showCreateModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-6 shadow-2xl animate-scale-in">
              ...
              <form onSubmit={handleCreateRole} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Role Title / Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-2">Initial Granted Permissions</label>
                  <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 bg-slate-950 rounded-xl border border-slate-800">
                    {permissions.map((p) => {
                      ...
                      return (
                        <label key={p.id} className="flex items-center gap-2 text-xs text-slate-300 p-1 hover:bg-slate-900 rounded cursor-pointer">
                          <input
                            type="checkbox"
                            checked={isChecked}
                            onChange={() => {
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md shadow-emerald-950/50"
                  >
